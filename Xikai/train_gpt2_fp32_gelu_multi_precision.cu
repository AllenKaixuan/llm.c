...
void attention_forward(float* out, float* preatt, float* att,
    ...
    cudaMemcpy(d_out, out, B * T * C * sizeof(float), cudaMemcpyHostToDevice);
    ...
    cudaMemcpy(preatt, d_preatt, B * NH * T * T * sizeof(float), cudaMemcpyDeviceToHost);
    cudaMemcpy(att, d_att, B * NH * T * T * sizeof(float), cudaMemcpyDeviceToHost);
    cudaMemcpy(inp, d_inp, B * T * 3 * C * sizeof(float), cudaMemcpyDeviceToHost);
...
__global__ void gelu_backward_kernel(__nv_bfloat16* dinp, const __nv_bfloat16* inp, const __nv_bfloat16* dout, int N) {
    ...
    if (idx < N) {
        ...
        dinp[idx] = local_grad * __bfloat162float(dout[idx]);
    }
}
...
void gpt2_forward(GPT2 *model, int* inputs, int* targets, size_t B, size_t T) {
    ...
    for (int l = 0; l < L; l++) {
        ...
        //printf(" attention_forward (took %f ms)\n",   time_elapsed_s * 1000);
