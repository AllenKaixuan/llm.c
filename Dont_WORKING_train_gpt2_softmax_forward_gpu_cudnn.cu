...
void softmax_kernel(const float* logits, float* probs, int N, int V, int Vp){
    cudnnHandle_t cudnn;
    ...
    // 关键：使用 cudnnSetTensor4dDescriptorEx 显式设置 stride
    // 形状：N x V x 1 x 1
    // strides: 
    //   - batch 之间的 stride = Vp
    //   - channel 之间的 stride = 1
    //   - H 和 W 都是 1，因此它们的 stride 也为 1（或者无所谓，同为 1）。
    checkCudnnErrors(
    cudnnSetTensor4dDescriptorEx(
        ...
        1,  // H
        1,  // W
        Vp, // strideN, 即相邻 batch（行）之间的步幅
        1,  // strideC, 即相邻 channel 之间的步幅
        1,  // strideH
        1   // strideW
    ));
...
void softmax_forward(float* probs, float* logits, int B, int T, int V, int Vp) {
    ...
    */
    ...
    double h2dMemcpyTime = (getTime() - startTime) * 1000;

    startTime = getTime();
    softmax_kernel(probs_device, logits_device, N, V, Vp);
    double kernelTime = (getTime() - startTime) * 1000;

    startTime = getTime();
    checkCudaErrors(cudaMemcpy(probs, probs_device, N * Vp * sizeof(float), cudaMemcpyDeviceToHost));
    double d2hMemcpyTime = (getTime() - startTime) * 1000;
    printf("softmax forward time is h2dMemcpy : %f kernel : %f d2hMemcpy : %f\n", h2dMemcpyTime, kernelTime, d2hMemcpyTime);
