...
// this kernel performs the GeLU operation in a memory-bound way
struct alignas(16) Packed128 {  
    ...
    __device__ const float& operator[](int index) const {
        return payload[index];
    }
    ...
    static constexpr const int size = sizeof(int4) / sizeof(float);
    float payload[size];
};

typedef Packed128 f128;

// load a Packed128 from an aligned memory address

__device__ Packed128 load128(const float* address) {
    return Packed128{*reinterpret_cast<const int4*>(address)};
}

// store a Packed128 to an aligned memory address

__device__ void store128(float* target, Packed128 value) {
...
__global__ void gelu_backward_kernel(float* dinp, const float* inp, const float* dout, const int N) {
    ...
    if (i < N) {
        f128 packed_dinp;
        f128 packed_inp = load128(inp + i);
        f128 packed_dout = load128(dout + i);
        for (int k = 0; k < packed_inp.size; ++k) {
            ...
            packed_dinp[k] = (float)(local_grad * (float)packed_dout[k]);
        }
...
__global__ void softmax_cross_kernel(float* losses, const int* targets, float* logits, float* probs, const float* dlosses, int N, int V, int Vp){
   ...
   //reduce to get the global max and sum
    ...
    for (int col = threadIdx.x; col < V; col += blockDim.x) {
        ...
        float indicator = (col == targets[row]) ? 1.0f : 0.0f;
        logits[row * Vp + col] = (val - indicator) * dloss;
    }
...
// change: 1) remove float4 2)add add_bias
...
void matmul_forward(float* out,
    ...
    */
    ...
    if (bias != NULL) {
        ...
        int grid_size = (B*T*OC + block_size - 1) / block_size;
...
void matmul_backward1(float* dinp, float* dweight, float* dbias,
    ...
    if (dbias != NULL) {
         ...
         matmul_backward_bias_kernel_naive<<<grid_size, block_size>>>(dbias, dout, B, T, OC);
    }
    cublasDestroy(cublas_handle);
}
...
void gpt2_forward(GPT2 *model, int* inputs, int* targets, int B, int T) {
    ...
    if (targets != NULL) {
        ...
        softmax_cross_forward(acts.output, acts.losses, NULL, model->targets, B, T, V, Vp);
        ////////////////////////////////////////////////////////////////////////////////////
        /*
...
void gpt2_backward(GPT2 *model) {
    ...
    */
    matmul_backward1(grads_acts.bt4c, grads.wte, NULL, acts.output, acts.lnf, params.wte, B, T, C, Vp);
    ...
    for (int l = L-1; l >= 0; l--) {
        ...
        // backprop this layer
        matmul_backward1(dl_bt4c, dl_fcprojw, dl_fcprojb, dresidual, l_fch_gelu, l_fcprojw, B, T, 4*C, C);
        gelu_backward(dl_bt4c, l_fch, dl_bt4c, B*T*4*C);
        matmul_backward1(dl_btc, dl_fcw, dl_fcb, dl_bt4c, l_ln2, l_fcw, B, T, C, 4 * C);
        // layernorm backward does += to the dresidual, so it correctly accumulates grad from the MLP block above
        layernorm_backward(dresidual, dl_ln2w, dl_ln2b, dl_btc, l_residual2, l_ln2w, l_ln2_mean, l_ln2_rstd, B, T, C);
        matmul_backward1(dl_btc, dl_attprojw, dl_attprojb, dresidual, l_atty, l_attprojw, B, T, C, C);
        ...
        matmul_backward1(dl_btc, dl_qkvw, dl_qkvb, dl_bt4c, l_ln1, l_qkvw, B, T, C, 3 * C);
