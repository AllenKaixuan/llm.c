...
void save_probs_to_file(const char *filename, float *probs, size_t rows, size_t cols, size_t V) {
    ...
    for (size_t i = 0; i < rows; i++) {
        for (size_t j = 0; j < V; j++) {
            fprintf(file, "%.10f", probs[i * cols + j]);
            if (j < V - 1) {
                fprintf(file, " ");
            }
        }
...
__global__ void softmax_kernel(const float* logits, float* probs, int N, int V, int Vp){
    ...
    for (int col = threadIdx.x; col < V; col += blockDim.x) {
        ...
        localMax = fmax(logits[row * Vp + col] , localMax_prev);
        localSum = localSum * expf(localMax_prev - localMax) + expf(logits[row * Vp + col] - localMax);
    }
    ...
    //broadcast, dont introduce lantency
    float maxVal = sdata[0];
    float sumVal = sdata[blockDim.x];

    for (int col = threadIdx.x; col < V; col += blockDim.x) {
        float val = expf(logits[row * Vp + col] - maxVal);
        probs[row * Vp + col] = val / sumVal;
...
void softmax_forward(float* probs, float* logits, int B, int T, int V, int Vp) {
    ...
    */
    ...
    float *logits_device, *probs_device;
...
clock_gettime(CLOCK_MONOTONIC, &start);
    ...
    softmax_kernel<<<gridSize, blockSize, sharedMemSize>>>(logits_device, probs_device, N, V, Vp);
...
#ifdef TIMER
printf("softmax forward time is h2dMemcpy : %f d2hMemcpy : %f kernel : %f\n", h2d_time_elapsed_s * 1000, d2h_time_elapsed_s * 1000, kernel_time_elapsed_s * 1000);
#endif 

#ifdef PRINT2FILE
    save_probs_to_file("GPU_VERSION_PROBS",probs,B*T,Vp,V);
#endif
