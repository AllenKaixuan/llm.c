...
//     cudaCheck(cudaMemcpy(d_out, out, B * T * OC * sizeof(float), cudaMemcpyHostToDevice));
//     // cudaCheck(cudaMemcpy(d_bias, bias, OC * sizeof(float), cudaMemcpyHostToDevice));
//     cudaCheck(cudaMemcpy(d_inp, inp, B * T * C * sizeof(float), cudaMemcpyHostToDevice));
//     cudaCheck(cudaMemcpy(d_weight, weight, C * OC * sizeof(float), cudaMemcpyHostToDevice));
    
//     int sqrt_block_size = 32;
//     // dim3 gridDim = dim3((B * T + sqrt_block_size - 1) / sqrt_block_size, (OC + sqrt_block_size - 1) / sqrt_block_size);
//     // dim3 blockDim(sqrt_block_size, sqrt_block_size);
//     // matmul_forward_kernel1<<<gridDim, blockDim>>>(d_out, d_inp, d_weight, d_bias, B, T, C, OC);
//     matmul_forward2(d_out, d_inp, d_weight, d_bias, B, T, C, OC, sqrt_block_size);
//     cudaMemcpy(out, d_out, B * T * OC * sizeof(float), cudaMemcpyDeviceToHost);
...
//     matmul_backward1(d_dinp, d_dweight, d_dbias, d_dout, d_inp, d_weight, B, T, C, OC);

//     cudaCheck(cudaMemcpy(dinp, d_dinp, B * T * C * sizeof(float), cudaMemcpyDeviceToHost));
//     cudaCheck(cudaMemcpy(dweight, d_dweight, OC * C * sizeof(float), cudaMemcpyDeviceToHost));
//     if(dbias != NULL) {
//     cudaCheck(cudaMemcpy(dbias, d_dbias, OC * sizeof(float), cudaMemcpyDeviceToHost));
//     }    
